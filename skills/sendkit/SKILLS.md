---
name: sendkit
description: Send Telegram messages from an agent through the SendKit MCP `telegram` tool, with the SendKit CLI (`@tb-eng/sendkit`) as a fallback. Use when a user asks to send a Telegram message, mentions SendKit, wants to interact with the SendKit toolset, asks to verify SendKit manually, or needs to choose between the SendKit MCP and CLI workflows.
---

# SendKit

SendKit sends Telegram messages. It exposes the same operation two ways, both backed by `@tb-eng/sendkit-core`:

- **MCP tool** (`sendkit` server → `telegram` tool) — preferred for agents.
- **CLI** (`@tb-eng/sendkit`, binary `sendkit`) — fallback when MCP is unavailable or for manual verification.
...
Run the CLI without a global install via `bunx @tb-eng/sendkit telegram <chatId> <message>` (or the `npx` equivalent).
